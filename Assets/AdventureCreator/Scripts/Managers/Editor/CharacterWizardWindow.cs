...
namespace AC
{
	...
	public class CharacterWizardWindow : EditorWindow
	{
		...
		public static void Init ()
		{
			...
			window.baseObject = Selection.activeGameObject;
			if (window.baseObject && window.baseObject.GetComponent<Char>()) window.baseObject = null;
		...
		private void OnGUI ()
		{
			...
			{
				if ((canChooseCharType && currentPage != PageType.CharType) || (!canChooseCharType && currentPage != PageType.BaseGraphic) && !IsFirstPerson)
				{
			...
			{
				if (ClickedBottomButton ((position.width - BottomButtonWidth) * 0.5f - 150, "Restart"))
				{
					RebuildOptions ();
					canChooseCharType = true;
					SetPage (PageType.CharType);
				}
				
				if (ClickedBottomButton ((position.width - BottomButtonWidth) * 0.5f + 150, "Close"))
				{
					Close ();
				}
		...
		private void ShowPage ()
		{
			...
			switch (currentPage)
			{
				...
				case PageType.Complete:
					GUI.Label (new Rect (Padding + 20, 100, position.width - Padding - Padding - 40, 40), charType.ToString () + " created. Check the '" + charType.ToString () + "' Inspector to set up animation and other properties, as well as modify any generated Colliders / Rigidbody components.");
					if (charType == CharType.Player && assignPlayerPrefabID < 0)
					{
						GUI.Label (new Rect (Padding + 20, 180, position.width - Padding - Padding - 40, 40), "To register this is as the default Player character, make it a prefab and assign it in the Settings Manager's 'Character' panel.");
					}
					break;
