			...
			if (showHotspot)
			{
				...
				if (hotspotIconDisplay != HotspotIconDisplay.Never)
				{
					...
					if (hotspotIcon == HotspotIcon.Texture)
					{
						...
						bool showSize = hotspotDrawing != ScreenWorld.WorldSpace;
						hotspotIconGraphic.ShowGUI (showSize ? CursorIconBase.IncludeSize.Always : CursorIconBase.IncludeSize.No, true, "Hotspot icon texture:", (KickStarter.cursorManager) ? KickStarter.cursorManager.cursorRendering : CursorRendering.Software, "AC.KickStarter.settingsManager.hotspotIconGraphic", "The icon to use for Hotspots");
					}
					
					if (hotspotDrawing == ScreenWorld.WorldSpace || hotspotIcon == HotspotIcon.UseIcon)
					{
